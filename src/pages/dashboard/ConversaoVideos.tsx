          ...
          <div className={`bg-black rounded-lg relative ${isFullscreen ? 'w-screen h-screen' : 'max-w-4xl w-full h-[70vh]'
            ...
            <div className={`w-full h-full ${isFullscreen ? 'p-0' : 'p-4 pt-16'}`}>
              <VideoJSPlayer
                ...
                onPlay={() => console.log('Video.js conversão iniciado')}
                onPause={() => console.log('Video.js conversão pausado')}
                onEnded={() => console.log('Video.js conversão finalizado')}
                onError={(error) => {
                  console.error('Erro no Video.js:', error);
                  toast.error('Erro ao carregar vídeo. Tente abrir em nova aba.');
                }}
                onReady={() => {
                  console.log('Video.js conversão pronto');
                }}
              />
            </div>
          </div>
